# -*- coding: utf-8 -*-
"""
Created on Mon Sep 18 21:39:15 2023
...
def createWeight(inputlen, outputlen):
    ...
    for i in range(inputlen):
        for j in range(outputlen):
            ...
            weightLayer[i][j] = random.uniform(-0.5, 0.5)
    return weightLayer
...
def createBias(inputlen):  # 輸入神經元個數
    ...
    for i in range(inputlen):
        ...
        biasLayer[i] = random.uniform(0, 0)
    return biasLayer
...
layer1Weight = createWeight(784, 400)
layer2Weight = createWeight(400, 100)
layer3Weight = createWeight(100, 4)
layer1Bias = createBias(400)
layer2Bias = createBias(100)
layer3Bias = createBias(4)
...
gradientHiddenLayer3 = [0] * 100
gradientHiddenLayer2 = [0] * 400
gradientHiddenLayer1 = [0] * 784
...
for epoch in range(500):
    ...
    for case in range(101):
        for number in range(3,5):
            imagePath = "number/{}/{}.png".format(number,case)
            image = Image.open(imagePath)
            input = np.array(image.getdata(), dtype=float)
            input = flaten(input)
            HiddenLayer1 = fullyConnected(input, layer1Weight, layer1Bias, 400)
            HiddenLayer1 = activationFunction(HiddenLayer1, "sigmoid")
            HiddenLayer2 = fullyConnected(HiddenLayer1, layer2Weight, layer2Bias, 100)
            HiddenLayer2 = activationFunction(HiddenLayer2, "sigmoid")
            finalLayer = fullyConnected(HiddenLayer2, layer3Weight, layer3Bias, 4)
            finalLayer = activationFunction(finalLayer, "sigmoid")
...
for number in range(3,5):
    imagePath = "number/{}/{}.png".format(number,number)
    image = Image.open(imagePath)
    input = np.array(image.getdata(), dtype=float)
    input = flaten(input)
    HiddenLayer1 = fullyConnected(input, layer1Weight, layer1Bias, 400)
